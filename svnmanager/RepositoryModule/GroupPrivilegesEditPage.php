...
class GroupPrivilegesEditPage extends TPage
{
	...
	public function onLoad($param)
	{
		...
		$repositoryid = $_GET['RepositoryID'];
		$s_repositoryid = $this->Module->Database->qstr($repositoryid);	
		$results = $this->Module->Database->Execute("SELECT * FROM groupprivileges WHERE repositoryid=" . $s_repositoryid . " ORDER BY path, groupid");
	...
	public function onRemovePrivileges($sender, $param)
	{
		...
		if(!$this->User->isAdmin())
		{
			...
			$priv = $this->Module->Database->Execute("SELECT * FROM groupprivileges WHERE id=" . $s_id);
